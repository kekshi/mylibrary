...
public class FingerprintCore {
    ...
    public FingerprintCore(Context context) {
        ...
        Log.d("FGP","fingerprint isSupport: " + isSupport);
        initCryptoObject();
    }
    ...
    private void initCryptoObject() {
        try {
            mCryptoObjectCreator = new CryptoObjectCreator(cryptoObject -> {
                mHandler.post(() -> startAuthenticate(cryptoObject));
                // 如果需要一开始就进行指纹识别，可以在秘钥数据创建之后就启动指纹认证
            });
        } catch (Throwable throwable) {
            Log.d("FGP","create cryptoObject failed!");
        }
    }
    ...
    private void startAuthenticate(FingerprintManager.CryptoObject cryptoObject) {
        ...
        try {
            ...
            try {
                ...
                Log.d("FGP","startAuthenticate，SecurityException"+e);
            }
        } catch (Throwable t) {
            Log.d("FGP","startAuthenticate，Exception"+t);
        }
    }

    private void notifyStartAuthenticateResult(boolean isSuccess, String exceptionMsg) {
        if (isSuccess) {
            Log.d("FGP","start authenticate...");
            if (mFpResultListener.get() != null) {
                mFpResultListener.get().onStartAuthenticateResult(true);
            }
        } else {
            Log.d("FGP","startListening, Exception" + exceptionMsg);
            if (mFpResultListener.get() != null) {
    ...
    private void notifyAuthenticationSucceeded() {
        Log.d("FGP","onAuthenticationSucceeded");
        mFailedTimes = 0;
    ...
    private void notifyAuthenticationError(int errMsgId, CharSequence errString) {
        Log.d("FGP","onAuthenticationError, errId:" + errMsgId + ", err:" + errString + ", retry after 30 seconds");
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void notifyAuthenticationFailed(int msgId, String errString) {
        Log.d("FGP","onAuthenticationFailed, msdId: " +  msgId + " errString: " + errString);
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void onFailedRetry(int msgId, String helpString) {
        mFailedTimes++;
        Log.d("FGP","on failed retry time " + mFailedTimes);
        if (mFailedTimes > 5) { // 每个验证流程最多重试5次，这个根据使用场景而定，验证成功时清0
            Log.d("FGP","on failed retry time more than 5 times");
            notifyAuthenticationError(msgId, helpString);
            return;
        }
        Log.d("FGP","onFailedRetry: msgId " + msgId + " helpString: " + helpString);
        cancelAuthenticate();
    ...
    private static FingerprintManager getFingerprintManager(Context context) {
        ...
        try {
            ...
            Log.d("FGP","get FingerprintManager failed " + t);
        }
